...
> 性质：新增规范正本 + 共用规则引擎 + 派生脚本 + E 项门禁升级。**VERSION 本批未递增**（3.5.15 由并发会话占号中，避免覆盖），落地时以本条目为准递增。

### 🔴 查出的口径打架与实现缺口

1. **同一件事两处口径相反**：根 SKILL 纪律 15 写「★ 强制」，虾镜模块 §6 纪律 13 写「**如保留草图需求**」——一字之差等于可选，正是这一处让草图层整层 0 交付而无人报警。
2. **项目侧从未实现**：`build_prompts_ep001.py` 没有任何 sketch 分支，全项目搜「黑白草图」零命中 → 0/51 是未实现，不是漏跑。
3. **`sketch_prompt` 不在 build-data-js 白名单里**：与 `voice_desc` 同坑——不透传则前端永远拿不到（实测写入 registry 后重建为 None）。
4. **我又造了一处双副本分叉（本批最重）**：`build-data-js.py` 有 scripts/ 与 templates/ 两份正本，我 3.5.13 只改了 `scripts/`（voice/layout 透传）、3.5.14 只改了 `templates/`（space_maps 集合 merge）——**两个方向都分叉，都是我的锅**。HEAD 上两份 md5 已不同（718460496a vs 873fe57157）。本批做并集并强制对齐。

### ➕ 变更

- 新增规范正本 `modules/xiajing-episodes/references/sketch-prompt-spec.md`：定位（构图先行验错层，不带风格/光/焦段/时长）、固定框架、五要素顺序、保留/剥离词表、可画性红线（含"必须出现侧别断言"、"画外角色不进草图"）、派生算法、与首帧/视频的分工表。
- 新增共用引擎 `scripts/sketch_rules.py`：词表 + `make()` 派生 + `validate()` 判据，**生成器与门禁共用一份**，杜绝规则第二副本。
- 新增派生器 `scripts/gen-sketch-prompts.py`：按规则从 `shots.json` 幂等派生（只补空、只修违规；人工修订若仍合规则保留）；补不出的显式写 `【待人工补姿态】`——不臆造、不静默降级成空镜。
- `check-script-fidelity.py` E 项升级为调用 `sketch_rules.validate()`；`--selftest` 增至 4 例（新增"框架齐但神态未剥离＋缺侧别"投毒，证明 §3.2/§四 判据不空转）。
- `scripts/build-data-js.py` + `templates/build-data-js.py`：透传 `sketch_prompt`；并抹平上述双向分叉（两份现逐字节相同，含 sketch/voice/layout/space_maps 四类透传）。
- SKILL.md 纪律 15 与虾镜纪律 13 改为**指向正本**，废止「如保留草图需求」措辞。

### 🔧 派生逻辑踩过的三个坑（dry-run 实测逼出来的）

1. 最初把姿态白名单当**准入闸门**（必须命中才保留）→ 白名单永远不全，"下锅/翻面/手收紧/嗅/打量"这类可画动作被成片丢掉，20/51 待人工。改为**黑名单剥离优先**（白名单只用于排序），降到 1/51。
2. 动作链用「→」分隔而未纳入切分 → "揉拧耳朵→眼神发怔"被一个 banned 词连坐整句丢弃。
3. 先剥括号再判非入画标记 → 「角色B（画外·不见人）」被剥成「角色B」**画进草图**；改为先按原始 token 判、再剥括号。另：单字「道/说/哗」误伤"道具/村道/哗然"，全部换为多字词。

### ✅ 回归

渔村 ep001 实跑派生：51/51 生成，**50 条直接合规、1 条（镜012）如实留 `【待人工补姿态】`**（其源字段确无可画动词）；`check-script-fidelity` 由「E 整层缺失」变为「E 规则违规 1 条」，`--selftest` 4 例全过；`build-data-js` 重建后 db 内 `sketch_prompt` **51/51 可见**（透传前会被白名单丢弃）；`check-template-hygiene` 首跑报 **P0 1**——`sketch_rules.py` 注释里写了本项目角色名，脱敏后复跑 P0 0/P1 0（**本轮刚建的门禁，第一个违规者是写它的人**）。

### 📎 证据

- `scripts/sketch_rules.py` / `scripts/gen-sketch-prompts.py` / `modules/xiajing-episodes/references/sketch-prompt-spec.md`
- `git show HEAD:scripts/build-data-js.py | md5sum` 与 `git show HEAD:templates/build-data-js.py | md5sum`（提交前 718460496a vs 873fe57157，提交后相同）
- 项目侧 `outputs/xiajing/ep001/shots.json.bak-sketch-*`（派生前备份）
...
1. **⑩ 声线判据是猜的**：虾塘 SKILL 只写"声线字段"四字、无键名，我列了五个候选键去查——结论蒙对（声线确实整层没做）但判据不成立；且实测 `build-data-js.py` 是白名单装配，未知字段直接丢弃（写进 registry 后重建 db 里为 `None`），前端声线页仍是"开发中"占位（上传/录制 disabled）。**我自己违反了刚写下的纪律 22 第②条。**
2. **⑪ 的"自动解除"是死路径**：条款写"`layout_ready = true` 后解除"，但全仓 grep 确认**没有任何脚本、流程、服务端会写这个位**——只有 check-assets 自己读。等于兜底永久生效，而我在 SKILL/CHANGELOG 里把它写成已闭环。
3. **投喂清单生成器把交付物改薄了**：用通用脚本重跑时覆盖了手工版，丢掉 4 条**逐场几何核对项**（方位/侧别/机位半区——恰是防"方位画反"的关键项），且未备份。9062B → 8647B。
4. **我把 `check-assets.py` 改成了两份不一致**（收尾自查追加发现，性质最重）：该门禁存在两份正本——`scripts/check-assets.py`（总指挥纪律 14 引用）与 `modules/xiatang-characters/scripts/check-assets.py`（虾塘模块引用）。用 `git show <ref>:<路径> | md5sum` 比对确认：3.5.10 与 3.5.11 期间两份逐字节相同（同为 `c3f8fddfb0`），而我的 3.5.12 只改了 `scripts/` 那份 → **同一门禁从两个入口跑会得出不同结论**，正是我自己在纪律 22 里要求防的事。本次把两份统一到同一内容提交，并在 SKILL.md 纪律 14 补「check-assets 双副本纪律」，附可复验的 md5 对比方法。

### 🔧 修法

- **声线域契约落地**：`xiatang-characters/SKILL.md` 新增「声线域契约」小节——唯一键名 `characters[].voice_desc`（+ `voice_ready`，缺省由 `assets/voices/<角色>.mp3` 是否存在推断）、构建须显式放行、门禁按此查、并**诚实声明 UI 生产者未接**（⑩ 会持续报，属预期非误报）。`build-data-js.py` 放行这两个字段；`check-assets.py` ⑩ 改为查契约，文案区分"未落库"与"部分缺"。
- **A2 解除条件改用盘上事实**：`layout_ready=true` **或** 约定路径 `assets/scenes/<场景名>-layout.png` 真实存在即解除；`build-data-js.py` 同步放行 `layout_image`/`layout_ready` 并按约定路径探测。`scene-assets.md` A2·5 措辞随之改。
- **投喂清单防覆盖**：`gen-topology-feedlist.py` 输出加 AUTO-GEN 标记；目标存在且无标记 → 拒绝覆盖（须 `--force`）；自动并入同目录 `拓扑图验收-项目专属.md`（或 `--extra-checks`）。项目侧已补回 18 条逐场核对项，清单 8647B → 11171B，比原手工版更全。

### ✅ 回归

`check-assets --selftest` 6 探针全过（含"干净不误报"）；期间自检**反过来抓到夹具写坏**——把基础描述并入 `dtxt` 导致"无门通向"用例连"有门"前提都没了、⑪ 正确豁免却被判失败，改为基础描述恒在、仅切换标记后全绿。真数据：`check-assets` 仍 4 条（3 陈旧指针 + 1 声线未落库，文案已改指契约）；`build-data-js` 重建后 db 内 `voice_desc/voice_ready/layout_image/layout_ready` 四字段均可见（此前会被丢弃）；`py_compile` 全过。

### 📎 证据

- `modules/xiatang-characters/SKILL.md`「声线域契约」；`scripts/build-data-js.py`（两处放行注释）
- `scripts/check-assets.py`（⑩ 契约化 / ⑪ 盘上解除 / selftest 夹具）
- `scripts/gen-topology-feedlist.py`（GEN_MARKER / EXTRA_DEFAULT / --force）
